...
func _process(delta):
	...
	var in_level = global_position.y + $Sprite.texture.get_height() / 2 < 600
	var game_manager = get_game_manager()
	var has_enough_money = get_game_manager().can_buy(cost)
	
	$Sprite.set_visible(in_level && can_place_tower && has_enough_money)
